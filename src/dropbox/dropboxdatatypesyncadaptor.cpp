...
DropboxDataTypeSyncAdaptor::~DropboxDataTypeSyncAdaptor()
{
}
...
void DropboxDataTypeSyncAdaptor::finalCleanup()
{
}

void DropboxDataTypeSyncAdaptor::errorHandler(QNetworkReply::NetworkError err)
{
    // Dropbox sends error code 204 (HTTP code 401) for Unauthorized Error
...
void DropboxDataTypeSyncAdaptor::sslErrorsHandler(const QList<QSslError> &errs)
{
    QString sslerrs;
...
QString DropboxDataTypeSyncAdaptor::clientSecret()
{
    if (!m_triedLoading) {
        loadClientIdAndSecret();
    }
...
void DropboxDataTypeSyncAdaptor::signIn(Accounts::Account *account)
{
...
void DropboxDataTypeSyncAdaptor::signOnError(const SignOn::Error &error)
{
...
void DropboxDataTypeSyncAdaptor::signOnResponse(const SignOn::SessionData &responseData)
{
    QVariantMap data;
