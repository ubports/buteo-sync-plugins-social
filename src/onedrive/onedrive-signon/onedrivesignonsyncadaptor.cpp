...
OneDriveSignonSyncAdaptor::OneDriveSignonSyncAdaptor(QObject *parent)
    : OneDriveDataTypeSyncAdaptor(SocialNetworkSyncAdaptor::Signon, parent)
{
    setInitialActive(true);
}

OneDriveSignonSyncAdaptor::~OneDriveSignonSyncAdaptor()
{
}

QString OneDriveSignonSyncAdaptor::syncServiceName() const
{
    return QStringLiteral("onedrive-sync"); // TODO: change name of service to onedrive-signon!
}

void OneDriveSignonSyncAdaptor::sync(const QString &dataTypeString, int accountId)
{
    // call superclass impl.
    OneDriveDataTypeSyncAdaptor::sync(dataTypeString, accountId);
}

void OneDriveSignonSyncAdaptor::purgeDataForOldAccount(int, SocialNetworkSyncAdaptor::PurgeMode)
{
    // Nothing to do.
}

void OneDriveSignonSyncAdaptor::finalize(int)
{
    // nothing to do
}

void OneDriveSignonSyncAdaptor::beginSync(int accountId, const QString &accessToken)
{
    Q_UNUSED(accessToken);
    refreshTokens(accountId);
}

Accounts::Account *OneDriveSignonSyncAdaptor::loadAccount(int accountId)
{
...
void OneDriveSignonSyncAdaptor::raiseCredentialsNeedUpdateFlag(int accountId)
{
    ...
    if (acc) {
        ...
        acc->setValue(QStringLiteral("CredentialsNeedUpdateFrom"), QVariant::fromValue<QString>(QString::fromLatin1("sociald-onedrive-signon")));
        acc->selectService(Accounts::Service());
        acc->syncAndBlock();
    }
}

void OneDriveSignonSyncAdaptor::lowerCredentialsNeedUpdateFlag(int accountId)
{
    Accounts::Account *acc = loadAccount(accountId);
    if (acc) {
...
void OneDriveSignonSyncAdaptor::refreshTokens(int accountId)
{
    Accounts::Account *acc = loadAccount(accountId);
    if (!acc) {
        return;
...
void OneDriveSignonSyncAdaptor::initialSignonResponse(const SignOn::SessionData &responseData)
{
...
void OneDriveSignonSyncAdaptor::forceTokenExpiryResponse(const SignOn::SessionData &)
{
    SignOn::AuthSession *session = qobject_cast<SignOn::AuthSession*>(sender());
    session->disconnect(this);
...
void OneDriveSignonSyncAdaptor::triggerRefresh()
{
...
void OneDriveSignonSyncAdaptor::refreshTokenResponse(const SignOn::SessionData &responseData)
{
    SignOn::AuthSession *session = qobject_cast<SignOn::AuthSession*>(sender());
    int accountId = session->property("accountId").toInt();
    session->disconnect(this);
...
void OneDriveSignonSyncAdaptor::signonError(const SignOn::Error &error)
{
    SignOn::AuthSession *session = qobject_cast<SignOn::AuthSession*>(sender());
    int accountId = session->property("accountId").toInt();
    session->disconnect(this);
